...
> ?> 在本节中，将分析踝关节的运动学。分析了力臂、肌肉长度、肌肉收缩速度和踝关节角度之间的关系，为Hill模型提供了所需的变量数据。
...
>
>  通过对踝关节电刺激过程的Simulink仿真，得到了肌肉骨骼模型下踝关节力矩与踝关节角度和速度之间的关系。采用非线性最小二乘法对非线性曲面进行拟合。该过程被简化为以脚踝角度和角速度为自变量的多项式Eq.13，其参数$P00=110.9$，$P10=-35.9$，$P01=-4.7$。下图显示了电刺激下脚踝主动扭矩的模拟结果。
>
> $$\begin{equation*} {F_m}\left({v,\theta } \right)\ = \ {P_{00}}\ + \ {P_{10}}*v\ + \ {P_{01}}*\theta \tag{13} \end{equation*}$$
>
> ![ 扭矩、踝关节角度和角速度之间的关系。](gao7-3158426-large.gif)
>
> 运动过程中关节之间摩擦的影响被忽略。电刺激对肌肉的扭矩输入主要包括电刺激激活器产生的主要力$\tau ^T$[27]、肌肉的粘性力$\tau ^V$和弹性力$\tau ^E$。总转矩表示为Eq.14，每个分量表示为Eq.15。参数如下表所示。
>
...
> ?> 在本节中，使用踝关节逆动力学建立踝关节的角度、加速度和扭矩之间的关系，以形成状态的闭环方程。从而为识别模型中的未知参数奠定了方程基础。
...
$$\begin{equation*} P(k|k)\ = \ \left({I - {K_k}{C_k}} \right)P(k|k - 1) \tag{28} \end{equation*}$$
 ...
 在Matlab中，Jacobian用于求解Jacobian矩阵：
...
假设采样点的个数为$N$，具体迭代过程如下：当$k=0$时，初始值设置为$X({00}) = E[X(0)] = {X_0}$，$P({00}) = E[(X(0) - {X_0}){(X(0) - {X_0})^T}] = {P_X}0$。迭代收敛过程如上图所示。
...
> ?> 在本节中，将模拟扭矩与实际扭矩值进行比较，以验证模型的有效性。使用EKF算法进一步识别未知参数。然后，用实际实验数据验证了模型的适用性。



> [!Note]  踝关节角度、力矩实验及仿真结果
>
> 一名志愿者被撤回以测量踝关节角度和扭矩数据。将实验数据与仿真结果进行对比，验证了拟合转矩模型的有效性。实验中使用了Biodex System 4（美国Biodex）的扭矩传感器系统。它在等距和等速条件下测量关节扭矩和角度信号。PC（i7-9700，16GB，SSD 512G）存储数据并进行相关数据分析。志愿者坐在可调节高度方向的匹配座位上。他们的脚踝与动力轴保持对齐。扭矩装置磨损，如下图所示。
>
> ![扭矩测量。](gao8-3158426-large.gif)
>
> 严格按照仪器操作规程进行测量。志愿者被要求多次重复从足底屈曲到背屈的动作，以收集踝关节的位置和力矩数据。使用Matlab进行数据处理，对比实际测量和仿真结果。扭矩τ和角度之间的关系和误差如下图所示。
>
> ![测量、模拟backsliding扭矩及其绝对误差。](gao9-3158426-large.gif)
>
> 上图显示了后退时踝角和踝关节背屈扭矩之间的关系。实测数据趋势与仿真数据一致，总体误差范围为±5（N/m），表明模型的可靠性。
>
> EKF算法的参数估计结果
>
> 实验平台由功能性电刺激器（来自MEDEL的MotionStim8），姿势传感器（IMU形式Delsys，物理治疗电极（LT-1生理电极，厚度1mm，尺寸40mm×40mm）和PC（i7-9700，16GB，SSD 512G）组成。固定电刺激电流的幅度和频率为25 mA和25 Hz，脉冲宽度从50 μs到110 μs不等。表IV中受试者1的参数被选为实验对象，被馈送到伪随机多级（PRML）序列中。记录脚踝的输出角度响应。然后，从惯性项和重力项得到主体1的模型状态方程。未知参数使用 EKF 算法进行识别。根据EKF算法在Matlab中制备M文件，并将实验数据作为识别数据导入。
>
> ![ EKF算法的识别结果。(a)使用EKF标识的状态变量。(b)使用EKF识别的模型参数。(c)识别参数的方差。(d)EKF预测结果。](gao10-3158426-large.gif)
>
> 状态变量根据 EKF 的步骤进行估计。算法收敛时状态变量和识别的未知参数是稳定的，提取的状态变量是识别的模型参数值。图10（a）显示了500个迭代数据点下的估计踝关节角度、踝关节角速度和激活状态。图10（b）显示了模型未知参数的识别结果。计算出的识别参数的方差如图10（c）所示。当识别的数据点达到 250 时，10 秒后，方差开始接近 0，证明参数开始收敛。当数据被识别为 250 个点时，EKF 观测校正功能终止。用最新的估计状态参数值预测电刺激的输入脉宽数据，当数据点为500时停止预测。EKF的预测结果如图10（d）所示。从时间点0到250的识别过程是EKF滤波器估计过程，它与实际的踝关节角度数据重叠，如图10（d）所示。250 点后，EKF 校正功能关闭。在时间点 250-500 提取预测角度。结果表明：预测角度的轨迹与实际角度的轨迹一致。但是，脉冲宽度的电平存在一定的误差。因此，进一步验证了模型的鲁棒性。通过改变输入电刺激脉冲宽度的类型和周期，对模型进行了进一步的验证和适应分析。
>
> 模型适用性和稳定性测试
>
> 设置输入电刺激的脉宽波形变化，即方波、正弦波、PRML序列和滤波随机噪声（FRN）序列，以进一步测试模型的适用性。FRN和PRML序列的变化周期分别为1秒和2秒。利用本研究建立的模型对踝关节角度进行预测，得到不同脉宽变化下的模型预测数据结果，如图11所示。
>
