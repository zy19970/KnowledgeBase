...
- ?> 给出了迁移学习的基本应用，如何实现迁移以及训练参数

- ?> 实验设计新奇多样，细节和表示方法可以借鉴
...
> ?> 总体概述-->之前做的工作-->现在做的工作-->与LSTM相比较(典型例子)-->运行时间说明(实时性)

> [!Note] 
>
> 在这项工作中，我们使用带有迁移学习技术的NMS求解器信息神经网络 (hybrid-ANN) 估计脚踝扭矩。除了实验信号，**混合神经网络模型还通过附加的基于物理的NMS信息特征**（即个体肌肉力和踝关节扭矩）来增强标准神经网络。具体而言，我们研究了所提出的混合模型的联合扭矩预测精度，并将其与**两个基线模型**(标准神经网络和NMS)在对象内和对象间预测方面进行了比较。在跨对象预测中，我们采用了转移学习技术来改进扭矩预测。总体而言，我们发现混合神经网络预测扭矩的准确性高于NMS和标准神经网络，特别是在采用迁移学习技术后的跨对象情况下。
>
> 肌电驱动的NMS模型和神经网络模型都是关节扭矩估计的常用方法，并且根据预测场景，每种模型都有自己的优点和局限性。与NMS模型相比，标准ANN模型具有很强的近似性[15]，[36]，但对于看不见的运动，可能会变得不太准确，因为它是一个黑箱模型，仅仅**旨在基于训练的运动研究输入和输出之间的关系**[4]，[38]。在这方面，一个目的是利用NMS和神经网络模型的优势；在先前的研究[19]中，我们通过将NMS解算器的物理特征集成到标准ANN中，提出了一种混合ANN，受试者内预测的初步结果表明，混合模型比独立的NMS和标准ANN更准确地估计关节扭矩。然而，预测关节力矩的受试者间模型仍然具有挑战性。
>
>因此，在当前的研究中，我们将NMS求解的ANN模型与迁移学习相结合，以进一步提高受试者内和受试者间预测踝关节扭矩的预测精度。我们发现，一般来说，混合ANN模型比NMS和标准ANN模型更准确地预测扭矩。我们将这一发现归因于这样一个事实，**即根据关节动力学的结构，生理特征（在混合ANN模型中计算的个体肌肉力）是关节扭矩估计的主要中间成分**。因此，结合这些生理特征可以提高标准ANN模型的预测精度，**特别是在遇到看不见的运动时**。例如，与混合ANN和NMS模型相比，在受试者内预测中，当在Gaitself情况下对看不见的运动缓慢行走进行测试时，标准ANN模型的扭矩预测最不准确，尤其是在终端站姿中接近峰值的跖屈扭矩。**峰值扭矩附近的巨大差异可能是因为经过训练的标准 ANN 模型对 EMG 变化和步行速度更敏感，并且来自其他速度的测试数据可能超出了训练模型的分布范围。**
>
> 值得注意的是，**我们选择了经典类型的人工神经网络，因为它的功能和近似精度[15]，[44]，[45]经常被应用于关节扭矩预测**。其他具有不同结构的ANN，如长短时记忆(LSTM)网络，可能会有不同的结果。**虽然由于LSTM网络的记忆结构，它可以在一段时间内记住模式**，但我们将NMS模型合并到hybrid-ANN模型中也具有类似的功能，如式(1)所述；NMS模型也有一种记忆功能，因为它使用来自 MTU 神经激活的前两个时间步长的信息。此外，LSTM网络通常需要更多的数据进行训练，这限制了它们在当前研究中的使用，**因为我们的数据集很小**。我们的目的是研究基于物理的NMS和人工神经网络的组合模型是否会提高踝关节扭矩预测的准确性，无论是在对象内预测还是在对象间预测。不同类型的神经网络可以在未来的工作中进一步分析。在执行时间方面，hybrid-ANN是NMS和standard-ANN的结合，**其执行时间主要取决于NMS模型所需的时间。人工神经网络正向推理的在线执行时间成本可以忽略不计。**正如Sartori等人[45]之前的研究所指出的，实时**肌电驱动的NMS模型的平均延迟大约为35毫秒**。因此，混合神经网络模型仍然适用于实时应用。然而，本研究也存在一些局限性。只有SOL, GM和TA激活信号和踝关节足底和背屈角被用来估计踝关节扭矩。髋关节和膝关节，在日常活动中也起着重要作用，会影响双关节肌肉(biarticular muscles)的肌肌腱动力学。可以进一步验证所提出的基于 NMS 求解器的 ANN 模型对髋关节和膝关节的扭矩预测。

## 参考文献

> L. Zhang, X. Zhu, E. M. G. Farewik and R. Wang, "Ankle Joint Torque Prediction Using an NMS Solver Informed-ANN Model and Transfer Learning," in IEEE Journal of Biomedical and Health Informatics, 2022, doi: 10.1109/JBHI.2022.3207313.
